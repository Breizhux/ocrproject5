...
"""
Pipeline de preprocessing et modèle de prédiction de turnover.
Adapté du notebook du Projet 4.
"""
...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def _compute_satisfaction_moyenne(self, X):
        ...
        return X[satisfaction_cols].mean(axis=1)
    ...
    def _create_family_conflict(self, X):
        """Calcule un score de conflit travail-famille (0-10)."""
        weights = (3, 2, 2, 2)
        ...
        marital_score = X['statut_marital'].map({
            'Célibataire': 0,
            'Marié(e)': 0.5,
            'Divorcé(e)': 1
        })
    ...
    def _create_recent_change_flag(self, X):
        """Crée un flag pour les changements récents négatifs."""
...
class SalaryRatioEncoder(BaseEstimator, TransformerMixin):
    """Encode les ratios salariaux par rapport aux médianes de poste."""
...
# Configuration des features (extrait de ton notebook)
COLUMNS_TO_DROP = [
...
def create_preprocessing_pipeline():
    ...
    categorical_label_pipeline = Pipeline([
        ('label_encoder', FunctionTransformer(label_encode_transform))
    ])
    ...
    categorical_map_pipeline = Pipeline([
        ('mapper', FunctionTransformer(map_frequency))
    ])
...
def create_full_pipeline():
    """Crée le pipeline complet avec feature selection et classifieur."""
    preprocessing = create_preprocessing_pipeline()

    # Paramètres optimaux trouvés dans ton notebook
    best_params = {
        'C': 0.1,
